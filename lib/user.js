...
exports.list = (cb) => {
  ...
  for (person in db) {
    //We push a copy. We cannot use the user method as that increments the nextUID.
    listOfUsers.push({
      name: db[person].name,
      pass: db[person].pass,
      uid : db[person].uid,
      admin : db[person].admin
    });
  }

  //Since there are no errors, we will pass undefined for success
...
exports.add = (u, cb) => {
  ...
  if (db[u.name] != undefined) {
      cb('Person with same name ' + u.name + ' ' + 'already exists', u);
  }
  else {
    var newUser = user(u.name, u.pass, u.admin);
    db[u.name] = newUser;
    u.uid = newUser.uid; //Deep copying for user
    cb(undefined, u);
  }
...
exports.addAnon = (cb) => {
  //Have not deep copied yet.
  var anon = user("Anonymous User #" + (nextUID + 1), "Anonymous User #" + (nextUID + 1), false);
  db[anon.name] = anon;

  var copy = {
    name: anon.name,
    pass: anon.pass,
    nextUID: anon.nextUID,
    admin: anon.admin
  }
